...
export const ModuleActions = ({ cmodule, courseId }) => {
  const [action, setAction] = useState(null);
  const [published, setPublished] = useState(cmodule?.active);
  ...
  async function handleSubmit(event) {
    ...
    try {
      switch (action) {
        case "change-active": {
          const activeState = await changeModulePublishState(cmodule.id);
        ...
        case "delete": {
          if (published) {
            ...
            await deleteModule(cmodule.id, courseId);
            // router.refresh();
